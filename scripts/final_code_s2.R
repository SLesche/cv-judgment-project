...
average_raters_per_cv_s2 <- cv_long %>% 
  count(lebenslauf_id) %>% 
  pull(n) %>% 
  mean()
...
n_raters_final_cleaned_s2 <- length(unique(final_data$CASE))
...
begründungen_plot_s2 <- ggplot(kriterien_paper_data, aes(x = prozentsatz, y = reorder(Inhaltliches_Kriterium, prozentsatz))) +
...
for (merkmal in merkmale) {
  ...
  icc_ergebnisse_s2[[merkmal]] <- icc_berechnung$results
}
...
# print(icc_ergebnisse_s2[["geschaetzte_Extraversion"]])

# icc_average(icc_ergebnisse_s2[["geschaetzte_Extraversion"]]$ICC[2], average_raters_per_cv)
# required_raters(icc_ergebnisse_s2[["geschaetzte_Extraversion"]]$ICC[2], 0.90)

# ICC-Tabelle für den Neurotizismus
# print("--- ERGEBNIS NEUROTIZISMUS ---")
# print(icc_ergebnisse_s2[["geschaetzter_Neurotizismus"]])
# icc_average(icc_ergebnisse_s2[["geschaetzter_Neurotizismus"]]$ICC[2], average_raters_per_cv)
# required_raters(icc_ergebnisse_s2[["geschaetzter_Neurotizismus"]]$ICC[2], 0.90)

# ICC-Tabelle für die Gewissenhaftigkeit
# print("--- ERGEBNIS Gewissenhaftigkeit ---")
# print(icc_ergebnisse_s2[["geschaetzte_Gewissenhaftigkeit"]])
# icc_average(icc_ergebnisse_s2[["geschaetzte_Gewissenhaftigkeit"]]$ICC[2], average_raters_per_cv)
# required_raters(icc_ergebnisse_s2[["geschaetzte_Gewissenhaftigkeit"]]$ICC[2], 0.90)

# ICC-Tabelle für die Verträglichkeit
# print("--- ERGEBNIS VERTRÄGLICHKEIT ---")
# print(icc_ergebnisse_s2[["geschaetzte_Vertraeglichkeit"]])
# icc_average(icc_ergebnisse_s2[["geschaetzte_Vertraeglichkeit"]]$ICC[2], average_raters_per_cv)
# required_raters(icc_ergebnisse_s2[["geschaetzte_Vertraeglichkeit"]]$ICC[2], 0.90)

# ICC-Tabelle für die Offenheit
# print("--- ERGEBNIS OFFENHEIT ---")
# print(icc_ergebnisse_s2[["geschaetzte_Offenheit"]])
# icc_average(icc_ergebnisse_s2[["geschaetzte_Offenheit"]]$ICC[2], average_raters_per_cv)
# required_raters(icc_ergebnisse_s2[["geschaetzte_Offenheit"]]$ICC[2], 0.90)

# ICC-Tabelle für den IQ
# print("--- ERGEBNIS IQ ---")
# print(icc_ergebnisse_s2[["geschaetzer_IQ"]])
# icc_average(icc_ergebnisse_s2[["geschaetzer_IQ"]]$ICC[2], average_raters_per_cv)
# required_raters(icc_ergebnisse_s2[["geschaetzer_IQ"]]$ICC[2], 0.90)
...
accuracy_cor_extra_s2 <- cor.test(final_data$geschaetzte_Extraversion, final_data$wahre_Extraversion)
accuracy_cor_vertr_s2 <- cor.test(final_data$geschaetzte_Vertraeglichkeit, final_data$wahre_Vertraeglichkeit)
accuracy_cor_gewiss_s2 <- cor.test(final_data$geschaetzte_Gewissenhaftigkeit, final_data$wahre_Gewissenhaftigkeit)
accuracy_cor_neuro_s2 <- cor.test(final_data$geschaetzter_Neurotizismus, final_data$wahrer_Neurotizismus)
accuracy_cor_offen_s2 <- cor.test(final_data$geschaetzte_Offenheit, final_data$wahre_Offenheit)
accuracy_cor_iq_s2 <- cor.test(final_data$geschaetzer_IQ, final_data$wahre_Intelligenz)
...
average_data_cors_s2 <- average_data_per_cv %>% 
  summarise(
...
m1_s2 <- lm(wahre_Intelligenz ~ education, data = average_data_per_cv)

m2_s2 <- lm(wahre_Intelligenz ~ mean_IQ + education, data = average_data_per_cv)

model_comp_s2 <- anova(m1_s2, m2_s2)
